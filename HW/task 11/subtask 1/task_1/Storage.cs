...
namespace task_1
{
    internal class Storage
    {
        ...
        public IEnumerable<Product> Except(Storage other)
        {
            HashSet<Product> result = new HashSet<Product>(this.products);

            return result.Except(other.products);
        }
        public IEnumerable<Product> Intersection(Storage other)
        {
            HashSet<Product> result = new HashSet<Product>(this.products);

            return result.Intersect(other.products);
        }
        public IEnumerable<Product> Union(Storage other)
        {
            HashSet<Product> result = new HashSet<Product>(this.products);

            return result.Union(other.products);
        }
    }
}
